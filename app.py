...
from functions.functions import add_comic, create_access_token, create_user, estructura_comic, estructura_personaje, pwd_context
...
m = Marvel('35475389d5c62cff1e1585c3a30847b2',
           '1fdbc9f56cc947679bacc8e70c2219c1e4698887')
...
def get_comics_or_characters(option: Optional[str] = None, word: Optional[str] = None):
    if option == 'personaje' and word:  # searchComic/personaje/palabra
        ...
        results = lista
        return results
    # searchComic/all/palabra/(personajes y comics)
    elif option == 'all' and word:
...
async def add_my_comic(comic: ItemComic, token: str = Depends(oauth2_scheme)):
    ...
    if len(comic_search[1]) > 0:  # Si hay resultados obtenemos los datos del comic
        ...
        object_comic = {"id_comic": id_comic, "title": title_comic,
                        "image": image_comic, "onsaledate": onsaledate_comic}
        # Buscamos el comic en nuestra DB
        # comic_exist=connection.db.comics.find({"id_comic":id_comic})

        # if comic_exist:
        comic = add_comic(object_comic)
        comic['user'] = {"email": token_bonito['email'],
                         "name": token_bonito['name']}
        connection.db.comics_user.insert_one(comic)
        response = {'message': 'Comic registrado correctamente',
                    "comic": json.loads(json.dumps(comic, default=str))}
        # else:
...
async def login(form_data: OAuth2PasswordRequestForm = Depends()):
    ...
    password_check = pwd_context.verify(
        form_data.password, user_exist['password'])
    if password_check:
        access_token = create_access_token(
            data={"email": user_exist["email"], "name": user_exist["name"]}, expires_delta=timedelta(minutes=30))
...
async def get_ny_comics(token: str = Depends(oauth2_scheme)):
    token_bonito = jwt.decode(token, SECRET_KEY, ALGORITHM)
